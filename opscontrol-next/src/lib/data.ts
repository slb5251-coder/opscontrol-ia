...
export async function loadDashboard(){
 ...
 const[tanksResult,summaryResult,alertsResult,operationsResult,vesselsResult,trucksResult,maintenanceResult,qhseResult]=await Promise.all([
  ...
  supabase.from('operations').select('id,vessel,client,product,activity,status,planned_quantity,executed_quantity,unit,scheduled_at').order('scheduled_at',{ascending:true}).limit(20),
  ...
  supabase.from('maintenance_orders').select('id,title,maintenance_type,priority,responsible,due_date,status,equipment(name)').order('created_at',{ascending:false}).limit(20),
